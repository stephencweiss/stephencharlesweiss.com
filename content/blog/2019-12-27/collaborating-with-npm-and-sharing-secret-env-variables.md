...
The Font Awesome team scopes their projects on NPM (`@fortawesome`<sup>[1](#footnotes)</sup><a id="fn1"></a>), and for the pro version, requires an `_authToken` in the `.npmrc` in order to download the packages. The effect is similar to configuring packages to be downloaded from private registries (something I've had to work with in the past as we use JFrog internally).

However, because we have a dozen or so active contributors to the project and I wanted to mitigate any disruption to other's development efforts, I needed a solution for communicating _how_ to use `font-awesome` going forward.
...
1. Gravel Road: The standard approch to managing environment variables. It's not fancy, and while it normally gets the job done, the ride can be bumpy, and sometimes you can pop a tire.
2. Tree-Lined Road: In theory, this road is a joy to ride down. Not only does it function, but it's pretty. Unfortunately, sometimes the trees fall down and block all traffic.
3. Five-Lane Highway: It may not be the most pleasing to look at, but once you're on, it's usually the fastest way to get to your destination. This mostly-automated solution steamrolls the most of the bumps out of the ride - and the pretty trees, but it's pragmatic.
...
If we require that a developer enter in the auth token _before_ running the install script, the communication burden is high.

On the other hand, if we commit this environment variable to our install script, we have committed the `_authToken` to our repository.
...
This would work... until a developer didn't get the memo and failed to update their `.bash_profile`. If that happened, they'd be greeted by the following error:
...
Before waving the white flag, I tried one last approach on the gravel road: a fake variable within the `.bash_profile`.

In our case, we are using `vault` (by HashiCorp) as a secret management system. That meant that if I could _something_ in place so that NPM didn't error out, in theory I could replace it by the time it was necessary (this was before I understood how `npm` locks down config filles).

To aid with this approach, I had a simple script for developers to modify their `.bash_profile`
...
With the gravel road seemingly destined to cause problems it did give me an idea. What if we could automate the whole process and make it so seamless for the developer that they would never even know that running `npm install` was doing something special? Effectively - what could happen if we could _automatically_ run the `pre-install` script?

Chicago, my home for the last 7 years, has beautiful historic tree-lined roads. In many ways, it's a normal road, but the trees provide plenty of eye candy - making what could be a mundane trip into a pleasant one. That's how I thought about this new approach.

To get there I would need to chnage the order of operations a bit. Instead of:
...
And, do it all with one script, `npm install`.
...
Returning to our aims, let’s see how we did:
...
I guess the trees are pretty until one falls and blocks the road, stopping all traffic. When that happens, you might regret taking the scenic route. And having a predictable disruption like a failed install is hardly the pleasant trip we wanted to provide to collaborators.
...
So now, once an engineer is aware that the first time they install they will need to run the `pre-install` script that's _all_ they need to do. Everything else is handled for them and it's done in a way that eliminates any need for storing sensitive API keys locally or manually typing them in.

## Riding Off Into The Sunset

Adding `font-awesome` to a shared project turned out to be quite a bit more than I was expecting up front. And while we're not riding in luxury, the solution I arrived at (with the help numerous friends and colleagues) achieves most of the aims I established up front:
